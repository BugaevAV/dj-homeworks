...
class AdvertisementSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if self.instance.status == data['status']:
            raise ValidationError(f'Текущее обьявление уже имеет статус {data["status"]}')
        elif 'status' in data and data['status'] =='OPEN' and open_qty:
            raise ValidationError('Превышен лимит открытых обьявлений!')
        elif self.context['view'].action == 'create' and open_qty:
